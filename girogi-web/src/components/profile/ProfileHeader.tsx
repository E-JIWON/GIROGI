...
export function ProfileHeader({
  ...
  return (
    <div className="border-b border-grey-200 bg-white p-6">
      ...
      {/* 닉네임 및 바이오 */}
      <div className="mb-4">
        <h2 className="text-lg font-bold text-grey-900">{user.nickname}</h2>
        {user.bio && <p className="mt-2 text-base text-grey-700">{user.bio}</p>}
      </div>
      ...
      {isOwnProfile ? (
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-grey-300 px-4 py-3 text-sm font-medium text-grey-900 transition-all hover:bg-grey-50"
        >
...
function StatItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col items-center">
      <div className="text-lg font-bold text-grey-900">{value}</div>
